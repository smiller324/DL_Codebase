...
function ac_opf_reference(ref)
    ...
    @variable(model, -ref[:branch][l]["rate_a"] <= pf[(l,i,j) in ref[:arcs]] <= ref[:branch][l]["rate_a"])
    # Add power flow variables q to represent the reactive power flow for each branch
    @variable(model, -ref[:branch][l]["rate_a"] <= qf[(l,i,j) in ref[:arcs]] <= ref[:branch][l]["rate_a"])
    ...
    for (i,bus) in ref[:bus]
        ...
        @constraint(model,
            sum(pf[a] for a in ref[:bus_arcs][i]) +                  # sum of active power flow on lines from bus i +
        ...
        @constraint(model,
            sum(qf[a] for a in ref[:bus_arcs][i]) +                  # sum of reactive power flow on lines from bus i +
    ...
    for (i,branch) in ref[:branch]
        ...
        p_fr = pf[f_idx]                     # p_fr is a reference to the optimization variable pf[f_idx]
        q_fr = qf[f_idx]                     # q_fr is a reference to the optimization variable qf[f_idx]
        p_to = pf[t_idx]                     # p_to is a reference to the optimization variable pf[t_idx]
        q_to = qf[t_idx]                     # q_to is a reference to the optimization variable qf[t_idx]
        # note: adding constraints to p_fr is equivalent to adding constraints to pf[f_idx], and so on
